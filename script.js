...
const TROUBLESHOOTING_DATA = [
  {
    ...
    problem:
      "Access Token 만료 시 클라이언트에서 지속적으로 401 에러가 발생하여 사용자 경험이 저하되었습니다.",
    cause:
      "Refresh Token을 활용한 재발급 로직이 클라이언트 Interceptor에 완전히 연결되지 않았습니다.",
    solution: [
      "Axios Interceptor 401 에러 핸들링 추가",
      "Refresh Token으로 새 Access Token 발급 로직 구현",
      "실패했던 기존 요청을 큐에 담았다가 재요청 처리",
    ],
    result:
      "Token 만료 시에도 사용자가 인지하지 못할 정도로 매끄러운 인증 연장 흐름을 구성했습니다.",
    lesson:
      "클라이언트와 서버 간 인증 토큰 흐름을 정확히 이해하고 예외 상황을 처리하는 방법을 익혔습니다.",
  },
  {
    id: "wd-2",
    project: "withday",
    ...
    problem:
      "사용자 수가 증가함에 따라 여행 일정 추천 API의 응답 속도가 현저히 느려졌습니다.",
    cause:
      "데이터베이스 조회 과정에서 불필요한 반복 쿼리가 발생하고 조인 기준이 명확하지 않았습니다.",
    solution: [
      "자주 검색되는 조건에 복합 인덱스 추가",
      "연관 데이터 조회 쿼리 구조 개선",
      "페이징 기준을 명확히 분리하여 응답 데이터 축소",
    ],
    result:
      "추천 API 응답 시간을 안정화하고 사용자가 기다리는 시간을 줄였습니다.",
    lesson:
      "백엔드 기능 구현은 동작 여부뿐 아니라 데이터 접근 방식까지 함께 검토해야 한다는 점을 체감했습니다.",
  },
  {
    id: "wd-3",
    project: "withday",
    ...
    problem:
      "일부 구글 계정으로 로그인 시 프로필 이미지가 표시되지 않거나 이메일 정보가 누락되었습니다.",
    cause:
      "외부 API 응답 필드가 항상 동일하다고 가정했고, 사용자가 정보 제공을 제한한 경우의 예외 처리가 부족했습니다.",
    solution: [
      "OAuth 응답 파싱 로직 개선",
      "필수 데이터 누락 시 기본값 제공 처리 추가",
      "추가 정보 입력 폼으로 유도하는 플로우 추가",
    ],
    result:
      "다양한 구글 계정 환경에서도 안정적인 로그인 및 회원가입 프로세스를 확립했습니다.",
    lesson:
      "외부 API 연동 시 응답 데이터의 다양성과 예외 케이스 처리의 중요성을 배웠습니다.",
  },
  {
    id: "wd-4",
    project: "withday",
    ...
    problem:
      "관리자 페이지에서 전체 사용자 목록을 조회할 때 브라우저 렌더링이 멈추는 현상이 발생했습니다.",
    cause:
      "페이지네이션 없이 다량의 데이터를 한 번에 클라이언트로 전송하고 렌더링했습니다.",
    solution: [
      "서버 사이드 페이지네이션 적용",
      "한 페이지당 조회 건수 제한",
      "검색 조건과 정렬 기준을 API 파라미터로 분리",
    ],
    result: "초기 로딩 속도를 줄이고 관리자 화면의 조작 안정성을 확보했습니다.",
    lesson:
      "대용량 데이터 처리 시 프론트엔드와 백엔드의 역할을 나누어 최적화해야 한다는 점을 익혔습니다.",
  },
  {
    id: "c2c-1",
    project: "c2c",
    ...
    problem:
      "카카오맵에 폴리곤을 그리기 위한 SIDO_MAP 데이터 파싱 중 메모리 사용량이 급격히 증가했습니다.",
    cause:
      "원본 데이터 크기가 커서 클라이언트에서 한 번에 파싱하기에 부담이 컸습니다.",
    solution: [
      "백엔드에서 지도 데이터를 지역 단위로 분리",
      "필요한 지역 데이터만 요청하도록 로딩 방식 변경",
      "좌표 데이터 구조를 화면 표시 목적에 맞게 정리",
    ],
    result: "지도 로딩 속도를 개선하고 브라우저 메모리 부담을 줄였습니다.",
    lesson:
      "공간 데이터처럼 큰 데이터는 화면에서 필요한 단위로 가공해 전달해야 한다는 점을 이해했습니다.",
  },
  {
    id: "c2c-2",
    project: "c2c",
    ...
    problem:
      "사용자가 페이지를 새로고침하면 이메일 인증 타이머가 초기화되어 인증 진행이 불안정했습니다.",
    cause:
      "타이머 상태를 클라이언트 메모리에만 저장하여 새로고침 시 상태가 사라졌습니다.",
    solution: [
      "인증 시작 시각을 서버에 기록",
      "클라이언트에서는 서버 기준 남은 시간을 계산",
      "만료 여부를 서버 응답과 함께 검증",
    ],
    result:
      "새로고침 시에도 정확한 남은 시간 표시와 인증 프로세스 유지가 가능해졌습니다.",
...
function renderTroubleshootingList() {
  ...
  const filteredItems = TROUBLESHOOTING_DATA.filter(
    (item) => item.project === currentProjectFilter,
  );
...
function renderTroubleshootingDetail() {
  ...
  const item = TROUBLESHOOTING_DATA.find(
    (entry) => entry.id === currentTroubleId,
  );
...
function setupDisabledLinks() {
  document
    .querySelectorAll('[aria-disabled="true"], .disabled-link')
    .forEach((link) => {
